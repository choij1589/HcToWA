...
from ROOT import TFile, EnableImplicitMT
...
if CHANNEL == "1E2Mu":
    ...
    OUTFILE = f"/root/workspace/HcToWA/SignalRegion_ver3/Outputs/{CHANNEL}/{args.mass}/{SAMPLE}.root"
    features = [
...
clf_fake = SelfNormDNN(len(features), 2)
clf_fake.load_state_dict(
    torch.load(
        f"Outputs/{CHANNEL}/{args.mass}/models/chkpoint_sig_vs_fake.pt"))
clf_fake.eval()
clf_ttX = SelfNormDNN(len(features), 2)
clf_ttX.load_state_dict(
    torch.load(f"Outputs/{CHANNEL}/{args.mass}/models/chkpoint_sig_vs_ttX.pt"))
clf_ttX.eval()
...
def select(evt, muons, electrons, jets, bjets, METv):
    ...
    if CHANNEL == "1E2Mu":
        ...
        if len(bjets) >= 1:
            ...
            if muons[0].Charge() + muons[1].Charge() == 0 and abs(
                    DiMuon.Mass() - 91.2) < 15.:
                return "ZFake"
            else:
                return False
...
def loop(evt, clf_fake, clf_ttX, syst, htool):
    ...
    if tight_flag:
        ...
        htool.fill_hist(f"{SAMPLE}/{REGION}/{syst}/score_ttX", score_ttX,
                        weight, 100, 0., 1.)
        htool.fill_hist3d(f"{SAMPLE}/{REGION}/{syst}/fake_ttX_mMM", score_fake,
                          score_ttX, DiMuon.Mass(), weight, 100, 0., 1., 100,
                          0., 1., 3000, 0., 300.)
        ...
        htool.fill_hist(f"fake/{REGION}/Central/score_ttX", score_ttX, w_fake,
                        100, 0., 1.)
        htool.fill_hist3d(f"fake/{REGION}/Central/fake_ttX_mMM", score_fake,
                          score_ttX, DiMuon.Mass(), w_fake, 100, 0., 1., 100,
                          0., 1., 3000, 0., 300.)
        ...
        htool.fill_hist(f"fake/{REGION}/Up/score_ttX", score_ttX, w_fake_up,
                        100, 0., 1.)
        htool.fill_hist3d(f"fake/{REGION}/Up/fake_ttX_mMM", score_fake,
                          score_ttX, DiMuon.Mass(), w_fake_up, 100, 0., 1.,
                          100, 0., 1., 3000, 0., 300.)
        ...
        htool.fill_hist(f"fake/{REGION}/Down/score_ttX", score_ttX,
                        w_fake_down, 100, 0., 1.)
        htool.fill_hist3d(f"fake/{REGION}/Down/fake_ttX_mMM", score_fake,
                          score_ttX, DiMuon.Mass(), w_fake_down, 100, 0., 1.,
                          100, 0., 1., 3000, 0., 300.)
