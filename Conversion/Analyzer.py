...
parser.add_argument("--region", "-r", default=None, required=True,
                    type=str, help="DY or ZG")
parser.add_argument("--channel", "-c", default=None, required=True,
                    type=str, help="1E2Mu or 3Mu")
...
if args.channel == "1E2Mu":
    ...
    BKGs = ["rare", "ttX", "VV", "fake"]
    MCs = ["DY", "ZG", "rare", "ttX", "VV"]
    SAMPLE_DIR = "/Users/choij/workspace/HcToWA/Samples/Selector/2017/Skim1E2Mu__/"
    OUTFILE = f"/Users/choij/workspace/HcToWA/Conversion/Outputs/{args.channel}/{args.region}/{SAMPLE}.root"
...
WEIGHTSYSTs = ["L1PrefireUp", "L1PrefireDown", "PUReweightUp", "PUReweightDown",
               "IDSFUp", "IDSFDown", "TrigSFUp", "TrigSFDown"]
...
def select(evt, muons, electrons, jets, bjets, METv):
    ...
    if args.channel == "1E2Mu":
        ...
        # 3. N(b) = 0
        # 4. |M(3l) - 91.2| < 10 GeV
        # 5. Veto |M(l+l-) - 91.2| < 10 GeV or M(l+l-) < 12 GeV
        ...
        # mass condition
        ZCand = muons[0] + muons[1] + electrons[0]
        OSSF = muons[0] + muons[1]

        if not abs(ZCand.Mass() - 91.2) < 10.:
            return False
        
        if not abs(OSSF.Mass() - 91.2) > 10.:
            return False
        ...
        pair1, pair2 = make_ospairs(muons)

        if not abs(ZCand.Mass() - 91.2) < 10.:
            return False

        if not (abs(pair1.Mass() - 91.2) > 10. and pair1.Mass() > 12.):
            return False

        if not (abs(pair2.Mass() - 91.2) > 10. and pair2.Mass() > 12.):
            return False
